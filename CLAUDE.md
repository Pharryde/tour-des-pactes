...
- `src/hooks/useGameState.ts` — **le cerveau de l'app** : tout l'état persistant (`localStorage`) + toute la logique métier (actions, progression, pactes) + la synchro cloud Supabase
- `src/components/` — écrans/composants de rendu uniquement (pas de logique métier lourde ; délèguent via callbacks reçus en props)
- `src/utils/` — fonctions pures sans état (pactes, animations, calculs de récompense/compétences) ; contient aussi `supabaseClient.ts` (client singleton) et `sauvegardeCloud.ts` (lecture/écriture du snapshot cloud)
